...
function updateSlides() {
	...
	const i = e.params,
		{ wrapperEl: r, slidesEl: a, size: n, rtlTranslate: l, wrongRTL: o } = e,
		d = e.virtual && i.virtual.enabled,
		c = d ? e.virtual.slides.length : e.slides.length,
		p = elementChildren(a, `.${e.params.slideClass}, swiper-slide`),
	...
	for (let r = 0; r < u; r += 1) {
		...
		if (
			((C = 0),
			p[r] && (a = p[r]),
			y && e.grid.updateSlide(r, a, u, t),
			!p[r] || 'none' !== elementStyle(a, 'display'))
		) {
			if ('auto' === i.slidesPerView) {
				...
				if (
					...
					C = e.isHorizontal()
						? elementOuterSize(a, 'width', !0)
						: elementOuterSize(a, 'height', !0);
				else {
					const e = s(n, 'width'),
						t = s(n, 'padding-left'),
						i = s(n, 'padding-right'),
				...
				(C = (n - (i.slidesPerView - 1) * T) / i.slidesPerView),
					i.roundLengths && (C = Math.floor(C)),
					p[r] && (p[r].style[t('width')] = `${C}px`);
			p[r] && (p[r].swiperSlideSize = C),
				f.push(C),
				i.centeredSlides
					? ((x = x + C / 2 + b / 2 + T),
					  0 === b && 0 !== r && (x = x - n / 2 - T),
	...
	if (
		...
		i.setWrapperSize && (r.style[t('width')] = `${e.virtualSize + T}px`),
		y && e.grid.updateWrapperSize(C, h, t),
		!i.centeredSlides)
	) {
		const t = [];
	...
	if ((0 === h.length && (h = [0]), 0 !== T)) {
		...
		p.filter((e, t) => !(i.cssMode && !i.loop) || t !== p.length - 1).forEach(
			(e) => {
				e.style[s] = `${T}px`;
			},
		);
	...
	if (
		...
		setCSSProperty(r, '--swiper-centered-offset-before', -h[0] + 'px'),
			setCSSProperty(
				r,
...
function updateSlidesClasses() {
	const e = this,
		...
		n = (e) => elementChildren(i, `.${s.slideClass}${e}, swiper-slide${e}`)[0];
	let l;
	if (
		(t.forEach((e) => {
			e.classList.remove(
	...
	if (l) {
		...
		let i = elementPrevAll(l, `.${s.slideClass}, swiper-slide`)[0];
...
function updateActiveIndex(e) {
	...
	if ((void 0 === d && (d = getActiveIndexByTranslate(t)), i.indexOf(s) >= 0))
		o = i.indexOf(s);
	else {
	...
	s.isHorizontal() ? (l = i ? -e : e) : (o = e),
		...
		r.cssMode
			...
			: r.virtualTranslate ||
			  ...
			  (a.style.transform = `translate3d(${l}px, ${o}px, 0px)`));
...
function translateTo(e, t, s, i, r) {
	...
	let c;
	if (
		((c = i && e > o ? o : i && e < d ? d : e), a.updateProgress(c), n.cssMode)
	) {
			...
			t.centeredSlides
				? a < e.loopedSlides - i / 2 ||
				  a > e.slides.length - e.loopedSlides + i / 2
					? (e.loopFix(),
					  (a = e.getSlideIndex(
							elementChildren(s, `${n}[data-swiper-slide-index="${r}"]`)[0],
					  )),
					  nextTick(() => {
							e.slideTo(a);
					  }))
		...
		Array.isArray(o.controller.control)
			...
			: o.controller.control instanceof o.constructor &&
			  ...
			  o.controller.control.loopFix({
					...e,
					slideTo:
						o.controller.control.params.slidesPerView === h.slidesPerView && s,
			  });
	}
	o.emit('loopFix');
}
...
function onTouchStart(e) {
	...
	if (a.noSwiping && (h ? closestElement(u, d) : d.closest(u)))
...
function onTouchMove(e) {
	...
	if (!i.isMoved) {
		if (
			(w && S && s.loopFix({ direction: s.swipeDirection }),
...
function onTouchEnd(e) {
	...
	for (
		...
		void 0 !== l[e + t]
			? u >= l[e] && u < l[e + t] && ((h = e), (m = l[e + t] - l[e]))
			: u >= l[e] && ((h = e), (m = l[l.length - 1] - l[l.length - 2]));
	}
	let f = null,
		v = null;
	r.rewind &&
	...
	mount(e) {
		...
		return (
			!r &&
				...
				((r = createElement('div', t.params.wrapperClass)),
				s.append(r),
